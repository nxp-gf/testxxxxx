...
std::vector<float*>
RunInference(std::vector<uint8_t> &in, int image_width, int image_height, int image_channels) {
  ...
  results.push_back(interpreter->typed_output_tensor<float>(0));
  results.push_back(interpreter->typed_output_tensor<float>(1));
  results.push_back(interpreter->typed_output_tensor<float>(2));
  results.push_back(interpreter->typed_output_tensor<float>(3));
  return results;
}
...
bp::list Recognize(int rows,int cols,bp::str img_data)
{
    ...
    std::vector<float*> results = RunInference(output,  width, height, channels);
    float *locations = results[0];
    float *classes = results[1];
    float *scores = results[2];
    float *detections = results[3];
    for (int i = 0; i < (int)*detections; ++i) {
        bp::dict res;
        float *rect = locations + i * 4;
        res["lable"] = labels[classes[i] + 1];
        res["x"] = rect[0];
        res["y"] = rect[1];
        res["w"] = rect[2];
        res["h"] = rect[3];
...
BOOST_PYTHON_MODULE(ObjectDetection)
{
