...
void Pnet::generateBbox(const pBox *score, const pBox *location, mydataFmt scale){
    //for pooling 
    int stride = 2;
    int cellsize = 12;
    int count = 0;
...
void Rnet::RnetImage2MatrixInit(pBox *pbox){
    pbox->channel = 3;
    pbox->height = 24;
    pbox->width = 24;
...
void Onet::OnetImage2MatrixInit(pBox *pbox){
    pbox->channel = 3;
    pbox->height = 48;
    pbox->width = 48;
...
mtcnn::mtcnn(std::string dir) : refineNet(dir), outNet(dir){
    modeldir = dir;
    Init();
}
...
enum FaceDetectionResult mtcnn::Detect(Mat &image, std::vector<FaceInfo> &faces){
