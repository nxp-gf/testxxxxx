...
void FeatureDB::load_feature()
{
    ...
    if (!inf.is_open()) {
        cout << "Error opening file, create it!" << endl;
		ofstream of(dbfile);
		return;
    }

    while (!inf.eof()) {
		std::string name, strFeature;

        std::getline(inf, strFeature);
		
		//cout << strFeature << endl;
	
        if (strFeature.length() < 1)
            continue;

        char *token = strtok((char *)strFeature.c_str(), ",");
        name = token;
